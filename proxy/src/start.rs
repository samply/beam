...
use shared::config::{CONFIG_PROXY, CONFIG_SHARED};
use shared::config_shared;
...
pub async fn main(config: Config, config_shared: config_shared::Config) -> anyhow::Result<()> {
    CONFIG_PROXY.set(config).expect("Setting config failed");
    CONFIG_SHARED.set(config_shared).expect("Setting config failed");
...
async fn init_crypto(config: Config, client: SamplyHttpClient) -> Result<(), SamplyBeamError> {
    let private_crypto_proxy = shared::config_shared::load_private_crypto_for_proxy(&config.priv_key_file, &config.proxy_id)?;
    ...
    let _public_info: Vec<_> =
        shared::crypto::get_all_certs_and_clients_by_cname_as_pemstr(&config.proxy_id)
            ...
            .collect();
    let (serial, cname) =
        shared::config_shared::init_public_crypto_for_proxy(private_crypto_proxy, &config.proxy_id).await?;
