...
use axum::{http::StatusCode, routing::get, Router, extract::State, Json};
use serde::Serialize;
...
async fn handler<'a>(
    ...
    let (statuscode, summary, status_vault) = match state.vault {
        ...
        VaultStatus::LockedOrSealed => (StatusCode::SERVICE_UNAVAILABLE, Verdict::Unhealthy, "sealed"),
        _ => (StatusCode::SERVICE_UNAVAILABLE, Verdict::Unhealthy, "unavailable")
    };
    let health_as_json = HealthOutput {
        summary,
        vault: HealthOutputVault { status: status_vault }
    };
